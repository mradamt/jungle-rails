...
RSpec.describe Order, type: :model do
  describe 'After creation' do
    ...
    it 'deducts quantity from products based on their line item quantities' do
      ...
      @order.line_items.new(product: @product1, quantity: 2, item_price: 1, total_price: 1)
      @order.line_items.new(product: @product2, quantity: 2, item_price: 1, total_price: 1)
      ...
      expect(@product1[:quantity]).to eq(8)   # -2
      expect(@product2[:quantity]).to eq(3)   # -2
